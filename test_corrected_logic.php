...
foreach ($monthlyMovements as $monthKey => $monthMovements) {
    ...
    if ($runningQty > 0 || $runningNilai > 0) {
        ...
        echo "Added opening balance for $monthKey: $runningQty Ekor = Rp " . number_format($runningNilai, 0) . "\n";
    }
    ...
    foreach ($monthMovements as $m) {
        ...
        if ($m->direction === 'in') {
            if ($m->ref_type === 'adjustment') {
                ...
                $dailyOutQty = 0;
                $dailyOutNilai = 0;
                echo "Processing adjustment on $dateStr: {$m->qty} Ekor to pembelian\n";
            } else {
                ...
                $dailyOutQty = 0;
                $dailyOutNilai = 0;
                echo "Processing purchase on $dateStr: {$m->qty} Ekor to pembelian\n";
            }
        } else {
            $dailyInQty = 0;
            $dailyInNilai = 0;
...
echo "Adjustment masuk ke kolom Produksi:\n\n";
