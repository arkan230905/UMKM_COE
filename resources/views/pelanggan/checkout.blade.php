...
<div class="container py-4">
    <h2 class="mb-4 text-dark">Checkout</h2>
    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-header bg-white">
                    <h5 class="mb-0 text-dark"><i class="bi bi-truck"></i> Data Pengiriman</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('pelanggan.checkout.process') }}" method="POST" id="checkoutForm">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="alamat_pengiriman" class="form-control @error('alamat_pengiriman') is-invalid @enderror" rows="3" required placeholder="Masukkan alamat lengkap dengan kode pos">{{ old('alamat_pengiriman') }}</textarea>
                        ...
                        <div class="mb-3">
                            <label class="form-label text-dark">No. Telepon <span class="text-danger">*</span></label>
                            <input type="text" name="telepon_penerima" class="form-control @error('telepon_penerima') is-invalid @enderror" value="{{ old('telepon_penerima', auth()->user()->phone) }}" required placeholder="08xxxxxxxxxx">
                        ...
                        <div class="mb-3">
                            <label class="form-label text-dark">Metode Pembayaran <span class="text-danger">*</span></label>
                        ...
                        <div class="mb-3">
                            <label class="form-label text-dark">Catatan (Optional)</label>
                            <textarea name="catatan" class="form-control" rows="2" placeholder="Catatan untuk penjual (optional)">{{ old('catatan') }}</textarea>
                        </div>
        ...
        <div class="col-md-4">
            <div class="card border-0 shadow-sm sticky-top" style="top: 20px;">
                <div class="card-header bg-white">
                    <h5 class="mb-0 text-dark"><i class="bi bi-receipt"></i> Ringkasan Pesanan</h5>
                </div>
                <div class="card-body">
                    @foreach($carts as $cart)
                    <div class="d-flex justify-content-between mb-2 pb-2 border-bottom">
                        <div>
                            <strong class="text-dark">{{ $cart->produk->nama_produk }}</strong>
                            <br>
                            <small class="text-muted">{{ $cart->qty }} x Rp {{ number_format($cart->harga, 0, ',', '.') }}</small>
                        </div>
                        <span class="text-dark">Rp {{ number_format($cart->subtotal, 0, ',', '.') }}</span>
                    </div>
                    @endforeach
