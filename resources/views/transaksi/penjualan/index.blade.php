                                ...
                                @forelse($pendingPenjualans as $key => $pending)
                                    <tr>
                                        ...
                                        @php
                                            ...
                                            // Gunakan jenis_pengiriman asli dari order jika tersedia
                                            if ($pending->order && in_array($pending->order->jenis_pengiriman, ['ambil_di_toko', 'kasir'])) {
                                                $jenisPengiriman = 'Ambil di Toko';
                                            } elseif ($actualMethod === 'tunai' || $paymentMethod === 'tunai' || strpos($catatanLower, 'ambil di toko') !== false || strpos($catatanLower, 'bayar di toko') !== false || strpos(strtolower($pending->catatan_pembayaran ?? ''), 'ambil di toko') !== false) {
                                        ...
                                        <td class="text-center">
                                            <div class="action-row">
                                                ...
                                                <form action="{{ route('transaksi.penjualan.approve', $pending->id) }}" method="POST" class="d-inline form-approve-transaksi" id="form-approve-{{ $pending->id }}">
                                                    @csrf
                                                    <button type="button" class="btn-minimal text-success btn-setuju-transaksi" 
                                                        data-id="{{ $pending->id }}" 
                                                        data-nomor="{{ $pending->nomor_transaksi }}"
                                                        data-pelanggan="{{ $pending->pelanggan->nama_pelanggan ?? $pending->nama_pelanggan ?? 'Umum' }}"
                                                        data-pembayaran="{{ $pending->payment_method ?? 'Cash' }}"
                                                        data-pemenuhan="{{ $jenisPengiriman }}"
                                                        data-total="{{ number_format($pending->grand_total ?? $pending->total, 0, ',', '.') }}"
                                                        data-produk="{{ json_encode($pending->details->map(function($det) { return ['nama' => $det->produk->nama_produk ?? 'Unknown', 'qty' => intval($det->jumlah ?? $det->kuantitas ?? 1)]; })->toArray()) }}">
                                                        <i class="fas fa-check"></i> Setuju
                                                    </button>
                                                </form>
                                                <form action="{{ route('transaksi.penjualan.reject', $pending->id) }}" method="POST" class="d-inline form-reject-transaksi" id="form-reject-{{ $pending->id }}">
                                                    @csrf
                                                    <button type="button" class="btn-minimal text-danger btn-tolak-transaksi" data-id="{{ $pending->id }}" data-nomor="{{ $pending->nomor_transaksi }}">
