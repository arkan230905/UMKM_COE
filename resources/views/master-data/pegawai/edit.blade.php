    ...
    <form action="{{ route('master-data.pegawai.update', $pegawai->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select class="form-select" id="kategori_tenaga_kerja" name="kategori_tenaga_kerja" required>
                ...
                <option value="BTKL" {{ old('kategori_tenaga_kerja', $pegawai->kategori_tenaga_kerja) == 'BTKL' ? 'selected' : '' }}>BTKL</option>
                <option value="BTKTL" {{ old('kategori_tenaga_kerja', $pegawai->kategori_tenaga_kerja) == 'BTKTL' ? 'selected' : '' }}>BTKTL</option>
            </select>
        </div>

        <div class="mb-3">
            <label for="gaji" class="form-label">Gaji</label>
            <input type="number" class="form-control" id="gaji" name="gaji" value="{{ old('gaji', $pegawai->gaji) }}" required>
        </div>
