...
function hitungBaris(row) {
    ...
    if (hargaPerSatuanUtama === 0) {
        alert('Bahan baku belum memiliki harga. Lakukan pembelian terlebih dahulu!');
        hargaDisplay.textContent = 'Belum ada harga';
        subtotalDisplay.textContent = 'Rp 0';
        hitungTotal();
        return;
    }
    ...
    if (satuan === satuanUtama) {
        ...
        hargaPerSatuan = (hargaPerSatuanUtama / faktorUtama) * faktorPilihan;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.bom-row').forEach(row => attachEvents(row));
});
</script>
@endsection
