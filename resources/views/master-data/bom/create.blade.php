...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadBTKLData(produkId) {
        fetch(`/api/get-available-btkl/${produkId}`)
            ...
            .then(data => {
                ...
                // Add event listeners for BTKL checkboxes
                document.querySelectorAll('input[name="selected_btkl[]"]').forEach(checkbox => {
                    checkbox.addEventListener('change', updateTotals);
                });
    ...
    function loadBOPData() {
        fetch(`/api/get-available-bop`)
            ...
            .then(data => {
                ...
                data.forEach(item => {
                    // Parse komponen_bop JSON
                    let komponenBop = [];
                    try {
                        komponenBop = typeof item.komponen_bop === 'string' 
                            ? JSON.parse(item.komponen_bop) 
                            : (item.komponen_bop || []);
                    } catch (e) {
                        console.error('Error parsing komponen_bop:', e);
                        komponenBop = [];
                    }
                    
                    // Build komponen display
                    let komponenHtml = '';
                    if (komponenBop.length > 0) {
                        komponenBop.forEach((komp, idx) => {
                            if (idx < 3) { // Show max 3 components
                                komponenHtml += `
                                    <div class="col-4">
                                        <div class="${idx < 2 ? 'border-end pe-2' : ''}">
                                            <small class="text-muted d-block">${komp.component || 'N/A'}</small>
                                            <strong class="text-dark">Rp ${parseFloat(komp.rate_per_hour || 0).toLocaleString('id-ID')}</strong>
                                        </div>
                                    </div>
                                `;
                            }
                        ...
                        komponenHtml = '<div class="col-12 text-muted">Tidak ada komponen</div>';
                    }
                            ...
                            <div class="card border-0 shadow-sm h-100" style="background: linear-gradient(135deg, #f5e6d3 0%, #f9f0e6 100%); border-left: 5px solid #a0826d !important;">
                                <div class="card-body p-4">
                                    <div class="form-check mb-0">
                                        <input class="form-check-input" type="checkbox" 
                                               name="selected_bop[]" value="${item.id}" id="bop_${item.id}"
                                               data-tarif="${item.bop_per_unit || item.total_bop_per_produk || 0}"
                                               style="transform: scale(1.2); margin-top: 8px;">
                                        <label class="form-check-label w-100" for="bop_${item.id}">
                                            <div class="row align-items-center">
                                                <div class="col-md-3">
                                                    <div class="d-flex align-items-center">
                                                        <div class="rounded-circle p-2 me-3" style="width: 50px; height: 50px; display: flex; align-items: center; justify-content: center; background-color: #a0826d;">
                                                            <i class="fas fa-cogs text-white"></i>
                                                        </div>
                                                        <div>
                                                            <h5 class="mb-1 fw-bold" style="color: #a0826d;">${item.nama_bop_proses || 'BOP Proses'}</h5>
                                                            <small class="text-muted">${komponenBop.length} komponen</small>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="col-md-6">
                                                ...
                                                <div class="col-md-2 text-center">
                                                    <div class="rounded p-3" style="background-color: rgba(160, 130, 109, 0.1);">
                                                        <small class="text-muted d-block">Total BOP</small>
                                                        <h4 class="mb-0 fw-bold" style="color: #a0826d;">Rp ${parseFloat(item.bop_per_unit || item.total_bop_per_produk || 0).toLocaleString('id-ID')}</h4>
                                                        <small style="color: #a0826d;">per Unit</small>
                                                    </div>
                                                </div>
                                                <div class="col-md-1 text-center">
                                                    <div class="badge text-white px-3 py-2 d-flex align-items-center justify-content-center" style="min-height: 40px; background-color: #a0826d;">
                ...
                // Add event listeners for BOP checkboxes
                document.querySelectorAll('input[name="selected_bop[]"]').forEach(checkbox => {
                    checkbox.addEventListener('change', updateTotals);
                });
    ...
    function clearAllData() {
        document.getElementById('bbb-container').innerHTML = '<div class="col-12 text-center text-muted">Pilih produk terlebih dahulu</div>';
        document.getElementById('btkl-container').innerHTML = '<div class="col-12 text-center text-muted">Pilih produk terlebih dahulu</div>';
        document.getElementById('bop-container').innerHTML = '<div class="col-12 text-center text-muted">Pilih produk terlebih dahulu</div>';
        updateTotals();
    }
});
</script>
