            ...
            <form action="{{ route('master-data.aset.update', $aset->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('kategori') is-invalid @enderror" id="kategori" name="kategori" required>
                        ...
                        <option value="Perlengkapan Habis Pakai" {{ old('kategori', $aset->kategori) == 'Perlengkapan Habis Pakai' ? 'selected' : '' }}>Perlengkapan Habis Pakai</option>
                    </select>
                    @error('kategori')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control @error('tanggal_beli') is-invalid @enderror" id="tanggal_beli" name="tanggal_beli" value="{{ old('tanggal_beli', $aset->tanggal_beli->format('Y-m-d')) }}" required>
                    @error('tanggal_beli')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
