...
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fw-bold mb-0 text-white">Edit Budget BOP</h2>
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-primary text-white">
            <h5 class="mb-0"><i class="fas fa-edit me-2"></i>Form Edit Budget BOP</h5>
        </div>
        <div class="card-body">
            <form action="{{ route('master-data.bop.update', $bop->id) }}" method="POST" id="bopForm">
                @csrf
                @method('PUT')
                ...
                <div class="alert alert-info mb-4" role="alert">
                    ...
                    <h5 class="mb-0 fw-bold text-primary" id="selected_akun_display">
                        {{ $bop->coa->kode_akun }} - {{ $bop->coa->nama_akun }}
                    </h5>
                </div>
                    ...
                    <select class="form-select bg-dark text-white @error('kode_akun') is-invalid @enderror" 
                        ...
                        @foreach($akunBeban as $akun)
                            <option value="{{ $akun->kode_akun }}" 
                                    ...
                                    {{ old('kode_akun', $bop->kode_akun) == $akun->kode_akun ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <div class="input-group input-group-lg">
                        ...
                        <input type="text" 
                               ...
                               id="budget_display" 
                               value="{{ old('budget') ? formatNumberClean(old('budget')) : number_format($bop->budget, 0, ',', '.') }}"
                               placeholder="Ketik nominal..."
                               style="color: #ffffff !important; font-size: 1.25rem; font-weight: 500;"
                               required>
                        <input type="hidden" name="budget" id="budget" value="{{ old('budget', $bop->budget) }}">
                    </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea class="form-control bg-dark text-white @error('keterangan') is-invalid @enderror" 
                              ...
                              style="color: #ffffff !important;">{{ old('keterangan', $bop->keterangan) }}</textarea>
                    @error('keterangan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
