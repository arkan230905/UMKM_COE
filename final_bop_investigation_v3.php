...
echo "=== FINAL BOP INVESTIGATION V3 ===\n\n";
...
try {
    ...
    if ($allBOP > 0) {
        ...
        echo "2. Checking BOP records by proses_produksi_id...\n";
        $bopByProses = \Illuminate\Support\Facades\DB::table('bop_proses')
            ->where('user_id', 1)
            ->select('proses_produksi_id', \Illuminate\Support\Facades\DB::raw('COUNT(*) as count'))
            ->groupBy('proses_produksi_id')
            ->get();
        
        echo "BOP records by proses_produksi_id:\n";
        foreach ($bopByProses as $row) {
            echo "  - Proses BTKL ID {$row->proses_produksi_id}: {$row->count} records\n";
        }
        ...
        if ($bopData->count() === 0 && $allBOP > 0) {
            ...
            echo "    - Wrong proses_produksi_id values\n";
        ...
        // Create sample BOP data for proses_produksi_id = 1 and 2
        $sampleBOP = [
            [
                'user_id' => 1,
