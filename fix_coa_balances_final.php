...
echo "FIXING COA BALANCES FINAL - CORRECT BUKU BESAR DATA\n";
echo "================================================\n";
...
echo "\n=== FIXING COA BALANCES WITH CORRECT VALUES ===\n";
...
foreach ($allCoas as $coa) {
    ...
    if (isset($coaBalances[$kodeAkun])) {
        ...
        if ($currentSaldo != $newSaldo) {
            echo "Fixing {$kodeAkun} - {$coa->nama_akun}:\n";
...
// Calculate expected neraca saldo from fixed COA
$expectedDebit = 0;
$expectedCredit = 0;
...
echo "\nCOA balances final fix completed!\n";
