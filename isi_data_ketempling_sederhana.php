...
echo "=== ISI DATA KETAMPLING SEDERHANA ===" . PHP_EOL;
...
try {
    ...
    foreach ($btklData as $btkl) {
        ...
        $existing = \Illuminate\Support\Facades\DB::table('bom_job_btkl as bjb')
            ->leftJoin('btkls', 'bjb.btkl_id', '=', 'btkls.id')
            ->leftJoin('jabatans', 'bjb.jabatan_id', '=', 'jabatans.id')
            ->where('bjb.bom_job_costing_id', $bomJobCosting->id)
            ->where('bjb.kode_proses', $btkl['kode_proses'])
            ->first();
        ...
        if (!$existing) {
            ...
            if ($btkl && $jabatan) {
                ...
                \App\Models\BomJobBTKL::create([
                    ...
                    'btkl_id' => $btkl->id,
                    'jumlah_pegawai' => $btkl['jumlah_pegawai'],
                    'waktu_pengerjaan' => $btkl['waktu_pengerjaan'],
