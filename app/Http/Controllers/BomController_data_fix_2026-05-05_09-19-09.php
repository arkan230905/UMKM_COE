...
class BomController extends Controller
{
        ...
        try {
            ...
            if ($bomJobCosting) {
                $btklDataRaw = \Illuminate\Support\Facades\DB::table('bom_job_btkl')
                    ...
                    ->where('bom_job_btkl.user_id', auth()->id())
    ...
    {
        try {
            // Get products that have biaya bahan data from bom_job_bbb
            // CRITICAL MULTI-TENANT: Use produk_id directly from bom_job_bbb
            $produkIds = \App\Models\BomJobBBB::where('user_id', auth()->id())
                ->pluck('produk_id')
                ->unique();
            
            // Get products that have biaya bahan data
    ...
    public function store(Request $request)
    {
        try {
            ...
            // Get or create BomJobCosting from bom_job_bbb data
            $bomJobCosting = BomJobCosting::where('produk_id', $validated['produk_id'])
                ->where('user_id', auth()->id())
                ->first();
            
            if (!$bomJobCosting) {
                // Create BomJobCosting from bom_job_bbb data
                $bbbData = \App\Models\BomJobBBB::where('user_id', auth()->id())
                ...
                $bomJobCosting = BomJobCosting::create([
                    ...
                    'total_bop' => 0,
                    'total_hpp' => $totalBBB,
                    'hpp_per_unit' => $totalBBB
                ]);
            }

            // Calculate totals from selected processes
            $totalBtkl = 0;
            $totalBop = 0;
            ...
            $prosesBtkl = ProsesProduksi::where('kapasitas_per_jam', '>', 0)
                ...
                ->map(function($proses) use ($selectedProsesIds) {
                    ...
                    if ($proses->bopProses) {
                        ...
                        // Get komponen BOP and normalize data
                        if ($proses->bopProses->komponen_bop) {
                            $komponenBop = is_array($proses->bopProses->komponen_bop) 
                                ? $proses->bopProses->komponen_bop 
                                : json_decode($proses->bopProses->komponen_bop, true);
