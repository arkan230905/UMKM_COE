...
class BahanPendukungObserver
{
    ...
    {
        // ✅ PERBAIKAN: Check if table exists before querying
        if (!\Schema::hasTable('bom_job_bahan_pendukung')) {
            Log::info('⚠️ Table bom_job_bahan_pendukung does not exist, skipping BOM update', [
                'bahan_pendukung_id' => $bahanPendukung->id,
                'nama_bahan' => $bahanPendukung->nama_bahan
            ]);
            return;
        ...
        try {
            ...
            Log::error('❌ Error processing BOM update for bahan pendukung', [
                'bahan_pendukung_id' => $bahanPendukung->id,
                'error' => $e->getMessage(),
    ...
    {
        // ✅ PERBAIKAN: Check if table exists before querying
        if (!\Schema::hasTable('bom_job_costings') || !\Schema::hasTable('bom_job_bahan_pendukung')) {
            Log::info('⚠️ BOM Job Costing tables do not exist, skipping biaya recalculation', [
                'produk_id' => $produk->id,
        ...
        try {
            ...
            Log::error('❌ Error recalculating product biaya bahan', [
                'produk_id' => $produk->id,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    private function handleDeletedBahanPendukungInBOMs(BahanPendukung $bahanPendukung)
    {
        // ✅ PERBAIKAN: Check if table exists before querying
        if (!\Schema::hasTable('bom_job_bahan_pendukung')) {
            Log::info('⚠️ Table bom_job_bahan_pendukung does not exist, skipping BOM processing', [
                'bahan_pendukung_id' => $bahanPendukung->id,
                'nama_bahan' => $bahanPendukung->nama_bahan
            ]);
            return;
        ...
        try {
            ...
            Log::error('❌ Error processing BOM deletion for bahan pendukung', [
                'bahan_pendukung_id' => $bahanPendukung->id,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    private function recalculateProductBiayaAfterDeletion(Produk $produk, BahanPendukung $deletedBahanPendukung)
    {
        // ✅ PERBAIKAN: Check if table exists before querying
        if (!\Schema::hasTable('bom_job_costings') || !\Schema::hasTable('bom_job_bahan_pendukung')) {
            Log::info('⚠️ BOM Job Costing tables do not exist, skipping biaya recalculation after deletion', [
                'produk_id' => $produk->id,
                'nama_produk' => $produk->nama_produk,
                'deleted_bahan_pendukung' => $deletedBahanPendukung->nama_bahan
            ]);
            return;
        }
        ...
        try {
            ...
            Log::error('❌ Error recalculating product biaya after deletion', [
                'produk_id' => $produk->id,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
