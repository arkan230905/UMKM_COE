...
echo "=== Fix Penggajian Controller Logic ===" . PHP_EOL;
...
echo PHP_EOL . "Mencari logika COA BTKL..." . PHP_EOL;

// Pattern to find the COA assignment logic
$patterns = [
...
foreach ($patterns as $index => $pattern) {
    if (preg_match($pattern, $newControllerContent, $matches)) {
        echo PHP_EOL . "✅ Found COA logic pattern " . ($index + 1) . PHP_EOL;
        echo "Old logic: " . substr($matches[0], 0, 100) . "..." . PHP_EOL;
        
        $newControllerContent = preg_replace($pattern, $replacements[$index], $newControllerContent);
        $updated = true;
        echo "✅ Replaced with new logic" . PHP_EOL;
        echo "New logic includes Bagian Gudang handling" . PHP_EOL;
        break;
    }
}
...
echo "✅ Fixed: PenggajianController logic untuk Bagian Gudang" . PHP_EOL;
