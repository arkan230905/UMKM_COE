...
$bahanBakus = DB::table('bahan_bakus')
    ->select('id', 'nama_bahan', 'saldo_awal', 'harga_satuan', 'coa_persediaan_id')
    ->get();
...
// Ayam Potong
$ayamPotong = DB::table('bahan_bakus')->where('nama_bahan', 'like', '%ayam potong%')->first();
if ($ayamPotong) {
...
// Ayam Kampung
$ayamKampung = DB::table('bahan_bakus')->where('nama_bahan', 'like', '%ayam kampung%')->first();
if ($ayamKampung) {
...
// Bebek
$bebek = DB::table('bahan_bakus')->where('nama_bahan', 'like', '%bebek%')->first();
if ($bebek) {
...
// Bubuk Bawang Putih
$bawangPutih = DB::table('bahan_pendukungs')->where('nama_bahan', 'like', '%bawang putih%')->first();
if ($bawangPutih) {
...
foreach ($coa115Items as $item) {
    ...
    echo "  " . $item->nama_bahan . ": " . $total . PHP_EOL;
}
